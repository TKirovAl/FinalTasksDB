# Info21 v1.0 - Документация для защиты
...
### 1. Валидация данных
- Триггеры проверяют корректность XP
- CHECK constraints на уровне таблиц
- Процедуры проверяют логику до вставки

### 2. Оптимизация
- Индексы на внешние ключи
- Составные индексы (peer, date)
- Эффективные JOIN'ы

### 3. Динамический SQL
- Используется в процедурах метаданных
- quote_ident() для безопасности
- EXECUTE для выполнения

### 4. Рекурсивные запросы
- CTE для обхода иерархии заданий
- UNION ALL в рекурсии
- Подсчет уровней вложенности

### 5. Аналитические функции
- ROW_NUMBER() для ранжирования
- PARTITION BY для группировки
- Window functions для агрегации

### 6. Обработка NULL
- COALESCE для значений по умолчанию
- NULLIF для предотвращения деления на 0
...
### Базовые:
1. Зачем нужен ENUM type check_status?
   - **Ответ**: Ограничивает возможные значения статуса, предотвращает ошибки

2. Почему "check" в кавычках в некоторых местах?
   - **Ответ**: check - зарезервированное слово PostgreSQL

3. Что делает CASCADE в FOREIGN KEY?
   - **Ответ**: Автоматически удаляет связанные записи

### Средние:
4. Как работает триггер xp_validation_trigger?
   - **Ответ**: Проверяет три условия перед вставкой XP: max limit, P2P success, Verter success

5. Разница между BEFORE и AFTER триггерами?
   - **Ответ**: BEFORE - до операции (может изменить/отменить), AFTER - после (для логирования)

6. Зачем нужен ROW_NUMBER() в функциях?
   - **Ответ**: Для ранжирования и выбора топ-N записей в группах

### Сложные:
7. Объясните рекурсивный CTE в get_task_hierarchy_count()
   - **Ответ**: Базовый случай - корневые задания, рекурсия - добавляет дочерние с увеличением уровня

8. Как работает PARTITION BY в аналитических функциях?
   - **Ответ**: Разбивает данные на окна для независимых вычислений в каждой группе

9. Почему используется JSONB в аудит триггере?
   - **Ответ**: Гибкое хранение структурированных данных, эффективный поиск, индексация

---

## Возможные улучшения

1. **Индексы**: Добавить GIN индексы на JSONB в audit таблице
2. **Партиционирование**: Разделить timetracking по датам
3. **Материализованные представления**: Для часто используемых агрегаций
4. **Stored procedures**: Добавить процедуры для массовых операций
5. **Логирование**: Расширенное логирование изменений
